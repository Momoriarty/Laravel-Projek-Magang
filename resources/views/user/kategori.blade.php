...
@section('title', strtoupper($kategori->nama_kategori))
    ...
    <main id="main">
        <section id="sectionId" class="sectionClass">
            <div class="container mt-5">
                <div class="row">
                    <h1 class="text-white text-center mb-3">Kategori : {{ strtoupper($kategori->nama_kategori) }}</h1>
                </div>
                <div class="row mb-4">
                ...
                <div class="row" id="searchResults">
                    @forelse ($tk as $no => $data)
                        <div class="col-lg-3 col-md-3 col-6 mb-5 template-card"
                            data-title="{{ $data->template->nama_template }}">
                            <div class="card h-100 shadow border-0">
                                <div class="portfolio-img">
                                    <div class="background-img"
                                        style="background-image: url('{{ asset('storage/template-images/' . $data->template->gambar) }}');">
                                    </div>
                                    <div class="img-overlay">
                                        <img src="{{ asset('storage/template-images/' . $data->template->gambar) }}"
                                            class="img-fluid" alt="">
                                    </div>
                                </div>
                                <div class="card-body p-4">
                                    <a class="text-decoration-none link-dark stretched-link"
                                        href="/code/{{ $data->template->id }}">
                                        <h3 class="card-title">{{ $data->template->nama_template }}</h3>
                                        <div class="text-muted">
                                            {{-- <i class="bi bi-person"></i>{{ $data->template->user->name }} --}}
                                        </div>
                                    </a>
                                </div>
                                <div class="card-footer p-4 pt-0 bg-transparent border-top-0">
                                    <div class="d-flex align-items-end justify-content-between">
                                        <div class="d-flex align-items-center">
                                            <div class="small">
                                                <form action="{{ route('code.update', $data->template->id) }}"
                                                    method="post">
                                                    @method('PUT')
                                                    @csrf
                                                ...
                                                <div class="text-muted" style="margin-top: 3px;">
                                                    <i class="bi bi-eye"></i> {{ $data->template->kunjungan }}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
